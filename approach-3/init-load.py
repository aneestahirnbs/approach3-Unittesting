...
class InitLoad:
    ...
    def main(self,argv):
        ...
        spark_context, sql_context = self.utility.get_spark_context(argv.env_context)
        try:
            ...
            data_with_columns = raw_df.selectExpr(
                *tuple(self.ref_utility.get_columns_with_table_prefix(DAILY_DATA)))  # .withColumn() , current_timestamp())
    ...
    #     renamed_cols = (['{0} AS {1}_{2}'.format(s, table_name, s) for s in self.ref_utility.get_file_schema('', '')])
    #     return renamed_cols
...
def start_app(args):
    ...
    init_load = InitLoad(ref_util, utility)
    init_load.main(args)
